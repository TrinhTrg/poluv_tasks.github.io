<div 
    x-data="{ 
        open: false, 
        selected: @entangle('category'),
        categories: @json($categories->pluck('name', 'id')->toArray()),
        getCategoryName(id) {
            return this.categories[id] || 'Unknown';
        }
    }" 
    class="relative"
>
    <button 
        @click="open = !open" 
        type="button" 
        class="flex items-center justify-between w-full min-w-[120px] bg-[#EAD6C0] dark:bg-slate-700 hover:bg-[#E0CCB7] text-gray-800 dark:text-white py-2 pl-3 pr-2 rounded-xl text-sm font-medium transition shadow-sm"
    >
        <span 
            x-text="selected === 'all' ? 'All Categories' : getCategoryName(selected)" 
            class="truncate max-w-[100px]"
        ></span>
        <svg 
            class="w-4 h-4 ml-2 transition-transform duration-200" 
            :class="open ? 'rotate-180' : ''" 
            fill="none" 
            viewBox="0 0 24 24" 
            stroke="currentColor"
        >
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
        </svg>
    </button>

    <div 
        x-show="open" 
        @click.outside="open = false" 
        x-transition 
        class="absolute mt-2 right-0 w-full min-w-[120px] bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-gray-100 dark:border-slate-700 z-[100] overflow-hidden" 
        style="display: none;"
    >
        <ul class="py-1 max-h-60 overflow-y-auto custom-scrollbar">
            <li 
                @click="selected = 'all'; open = false" 
                class="px-3 py-2 text-sm cursor-pointer hover:bg-pink-50 dark:hover:bg-slate-700 text-gray-700 dark:text-gray-300" 
                :class="selected === 'all' ? 'font-bold text-pink-600 dark:text-pink-400' : ''"
            >
                All Categories
            </li>
            @foreach($categories as $cat)
                <li 
                    @click="selected = '{{ $cat->id }}'; open = false" 
                    class="px-3 py-2 text-sm cursor-pointer hover:bg-pink-50 dark:hover:bg-slate-700 text-gray-700 dark:text-gray-300 flex items-center gap-2" 
                    :class="selected == '{{ $cat->id }}' ? 'font-bold text-pink-600 dark:text-pink-400' : ''"
                >
                    <span class="w-3 h-3 rounded-full" style="background-color: {{ $cat->color }}"></span>
                    {{ $cat->name }}
                </li>
            @endforeach
        </ul>
    </div>
</div>
